...
(() => {
  ...
  var BuildWeb = class {
     ...
     */
    onSetup(task) {
      task.config.ignoreFilesInBin = task.config.ignoreFilesInBin.filter((item) => item !== "resources/**/*");
      console.log("IEditorEnv.BuildWeb:用插件在Web发布流程中重置了Bin目录复制时的排除文件规则");
      return;
    }
  };
  __name(BuildWeb, "BuildWeb");
  BuildWeb = __decorateClass([
